...
bool gc::SameThreadMarkAndSweep::PerformFullGC() noexcept {
    ...
    mm::ObjectFactory<gc::SameThreadMarkAndSweep>::FinalizerQueue finalizerQueue;
    {
        ...
        gcHandle.extraObjectsUsageBefore(extraObjectsDataFactory.GetSizeUnsafe(), extraObjectsDataFactory.GetTotalObjectsSizeUnsafe());
        ...
        GCLog(epoch_, "Swept in %" PRIu64 " microseconds", timeSweepUs - timeSweepStartUs);

        gcHandle.heapUsageAfter(objectFactory_.GetObjectsCountUnsafe(), objectFactory_.GetTotalObjectsSizeUnsafe());
        gcHandle.extraObjectsUsageAfter(extraObjectsDataFactory.GetSizeUnsafe(), extraObjectsDataFactory.GetTotalObjectsSizeUnsafe());
