...
internal class KtFirExpressionInfoProvider(
    ...
    private fun doesParentUseChild(parent: PsiElement, child: PsiElement): Boolean {
        return when (parent) {
            ...
            // UNLESS the it's an expression body/lambda of type Unit.
            ...
            // When expressions use the subject expression _unless_ the only branch in the
            // when is an `else`.
            is KtWhenExpression ->
 ...
 *  returns Unit UNLESS the function body is an expression body and the body is of type Unit;
 */

private fun doesNamedFunctionUseBody(namedFunction: KtNamedFunction, body: PsiElement): Boolean {
    return !returnsUnit(namedFunction) ||
            when {
