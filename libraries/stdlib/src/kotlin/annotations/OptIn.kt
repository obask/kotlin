 ...
 * ### Type scopes
 *
 * A type is considered to be as requiring opt-in if either the outer declaration (class or interface) requires opt-in
 * or any of its type parameters require opt-in.
 ...
 * If the base declaration requires an opt-in, it is propagated to all overrides in inheritors of
 * the base declaration, triggering an opt-in warning on any attempt to override such declaration
 * without `OptIn` or `Marker` annotations applied to the override.
 *
